...
function new_twitter_update_notice() {
    if (!get_transient('new-twitter-update')) {
        if (!empty($_GET['nextendtwitterconnect']) && $_GET['nextendtwitterconnect'] == 'new-twitter-update') {
            set_transient('new-twitter-update', true, WEEK_IN_SECONDS);
            return;
        }
        ?>
        <div class="nextend-twitter-connect-dismiss notice notice-warning is-dismissible">
        	<p>Nextend Twitter Connect is discontinued and <strong>Nextend Social Login</strong> handles Twitter login in the future. Please install and enjoy <strong>Nextend Social Login</strong>. The current settings of Nextend Twitter Connect will be imported to Nextend Social Login.
            <br><br>
        ...
        <script>
			jQuery(function ($) {
                $(document).ready(function () {
                    $('.nextend-twitter-connect-dismiss .notice-dismiss').on('click', function () {
                        window.location = window.location + (window.location.href.indexOf('?') > 0 ? '&' : '?') +
                            'nextendtwitterconnect=new-twitter-update';
                    });
                });
            });
        </script>
...
add_action('admin_notices', 'new_twitter_update_notice');
